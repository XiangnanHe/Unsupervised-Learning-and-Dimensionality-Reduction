...
class KMeansTestCluster():
    ...
    def run(self):
        ...
        for k in self.clusters:
            ...
            if k == self.targetcluster and self.stats:
                ...
                #print model.cluster_centers_
                print cdist(self.X, model.cluster_centers_, 'cityblock').shape

            #print np.min(np.square(cdist(self.X, model.cluster_centers_, 'euclidean')), axis = 1)
            min = np.min(np.square(cdist(self.X, model.cluster_centers_, 'cityblock')), axis = 1)
            print "### cluter ", k
    ...
    def plot(self, meandist, homogeneity, completeness, rand, silhouettes):
            ...
            np.savetxt("LR_KMeansTestCluster_SSE_output.csv", meandist, delimiter = ",")
            #plt.show()
            ...
            np.savetxt("LR_KMeansTestCluster_Homogeneity_output.csv", homogeneity, delimiter = ",")
           # plt.show()
            ...
            np.savetxt("LR_KMeansTestCluster_Completeness_output.csv", completeness, delimiter = ",")
            plt.show()
